...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        let wind = Double.random(in: -6...6)
        physicsWorld.gravity = CGVector(dx: wind, dy: -9.8)
        let windDescription: String
        if wind < 0.0 {
            windDescription = String(format: "%.1f wind to the left", abs(wind))
        } else if wind > 0.0 {
            windDescription = String(format: "%.1f wind to the right", abs(wind))
        } else {
            windDescription = "No wind"
        }
        
        viewController?.windLabel.text = windDescription
    ...
    func destroy(player: SKSpriteNode) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            ...
            if self.player1Score >= 3 || self.player2Score >= 3 {
