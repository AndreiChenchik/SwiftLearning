...
class GameScene: SKScene {
    ...
    let enemyProbability = 6
    let enemyXPositionRange: ClosedRange<Int> = 64...960
    let enemyYPosition: Int = -128
    let enemyVelocityMultiplier = 40
    let enemyFastVelocityMultiplier = 45
    let enemyAngularVelocityRange: ClosedRange<CGFloat> = -3...3
    let enemyYVelocityRange: ClosedRange<Int> = 24...32
    let enemyXVelocityGroupRanges: [(position: CGFloat, range: ClosedRange<Int>)] = [
        (256, 8...15),
        (512, 3...5),
        (768, 3...5),
        (0, 8...15)
    ...
    func endGame(triggerByBomb: Bool) {
        ...
        run(SKAction.playSoundFileNamed("youDied.mp3", waitForCompletion: false))
        ...
        let gameOverOverlay = SKShapeNode(rectOf: CGSize(width: 1040, height: 800))
        gameOverOverlay.position = CGPoint(x: 512, y: 384)
        gameOverOverlay.fillColor = SKColor.black
        gameOverOverlay.alpha = 0
        gameOverOverlay.zPosition = 1
        
        addChild(gameOverOverlay)
        
        let fadeInAction = SKAction.fadeIn(withDuration: 2.5)
        gameOverOverlay.run(fadeInAction)
    }
    ...
    func createEnemy(forceBomb: ForceBomb = .random) {
        ...
        var enemyType = Int.random(in: 0...enemyProbability)
        if forceBomb == .never {
            enemyType = 1
        ...
        if randomPosition.x < enemyXVelocityGroupRanges[0].position {
            randomXVelocity = Int.random(in: enemyXVelocityGroupRanges[0].range)
        } else if randomPosition.x < enemyXVelocityGroupRanges[1].position {
            randomXVelocity = Int.random(in: enemyXVelocityGroupRanges[1].range)
        } else if randomPosition.x < enemyXVelocityGroupRanges[2].position {
            randomXVelocity = -Int.random(in: enemyXVelocityGroupRanges[2].range)
        } else {
            randomXVelocity = -Int.random(in: enemyXVelocityGroupRanges[3].range)
        }
        ...
        if enemy.name == "enemyFast" {
            enemy.physicsBody?.velocity = CGVector(dx: randomXVelocity * enemyFastVelocityMultiplier, dy: randomYVelocity * enemyFastVelocityMultiplier)
        } else {
