...
class SelectionViewController: UITableViewController {
    ...
    func loadImage(_ item: String) -> UIImage {
        // find the image for this cell, and load its thumbnail
        let currentImage = item
        let imageRootName = currentImage.replacingOccurrences(of: "Large", with: "Thumb")
        guard let path = Bundle.main.path(forResource: imageRootName, ofType: nil) else { fatalError("Image can't be found") }
        guard let original = UIImage(contentsOfFile: path) else { fatalError("Image can't be loaded") }
